...
namespace Kreditstatus.Handlers
{
        ...
        IHandleMessages<VerificerAtOplysningerErBlevetOpdateret>
    {
        ...
        {
            var friskeStatusser = Data.Statusser.Where(s => s.Hvornår >= DateTime.UtcNow - TimeSpan.FromMinutes(10));

            if (!friskeStatusser.Any())
            {
                Console.WriteLine("Æi, ingen friske statusser for {0}: {1}", Data.Cpr, string.Join(", ", friskeStatusser.Select(s => s.Hvornår)));
            }

            return friskeStatusser.Any();
        }
    }
